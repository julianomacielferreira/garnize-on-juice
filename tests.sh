...
PAYMENTS_ENDPOINT="http://localhost:9999/payments"

PAYMENTS_SUMMARY_ENDPOINT="http://localhost:9999/payments-summary"
...
for i in $(seq 1 $NUM_REQUISICOES); do
  curl --location "$PAYMENTS_ENDPOINT" \
    --header 'Content-Type: application/json' \
    --data "$PAYLOAD" &
  
  sleep 0.05
  
  curl --location "$PAYMENTS_SUMMARY_ENDPOINT?from=$FROM_DATE&to=$TO_DATE" > /dev/null &
